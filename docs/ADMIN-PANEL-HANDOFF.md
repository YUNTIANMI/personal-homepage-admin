...
```
请基于当前项目开发后台管理系统。本项目是 luoji-admin，由前台项目 luoji-home 复制而来，两者共用同一个 Supabase 数据库；项目交接信息见 docs/ADMIN-PANEL-HANDOFF.md（请先完整阅读）。

我的决定：
1. 形态：本项目本身就是后台（独立于前台项目），引入 react-router-dom 规划 /login 与 /admin/* 路由
2. 认证：Supabase Auth（邮箱 + 密码，单管理员）
3. 数据与权限：与前台共用同一个 Supabase 实例；RLS 从「anon 全量读写」收紧为「anon 只读 + authenticated 可写」，
   务必保留 anon 对 posts 的 select 权限（仓库内的保活工作流依赖它）
4. 范围边界：前台项目 luoji-home 不在本项目范围内，不要修改它；
   它的管理按钮在 RLS 收紧后会写入失败，属已知预期，不是本项目要修的问题
5. 第一阶段范围：登录 + 仪表盘 + 文章管理（增删改查/搜索/批量删除）+ 项目管理 + Markdown 编辑预览
6. 视觉：复用现有设计 Token 与 components/ui.tsx，保持同一套视觉语言，桌面端遵循根字号阶梯规范；
   后台采用「左侧边栏 + 顶栏 + 内容区」布局，管理页以表格为主（信息密度高于前台）
7. 约束：不重做设计，不改配色与字体，保持现有响应式与无障碍规范（字号用 rem 不用固定 px）

请先给出实施计划（文件与路由结构 + 页面清单 + 数据与 RLS 变更 + 风险点），确认后再动手写代码。
```
